...
public class SecuConfig {
    ...
    private List<Integer> mCryptoList = new ArrayList();
    private List<Integer> mPrivacyCryptoList = new ArrayList();
    ...
    public boolean isTripleDes() {
        if (this.mCryptoList.isEmpty()) {
            return true;
        }
        int maxCryptoToken = 0;
        for (int i = 0; i < this.mCryptoList.size(); i++) {
            if (maxCryptoToken < this.mCryptoList.get(i).intValue()) {
                maxCryptoToken = this.mCryptoList.get(i).intValue();
            }
        }
        return maxCryptoToken == 0;
    }

    public boolean isAes256Gcm() {
        if (this.mCryptoList.isEmpty()) {
            return false;
        }
        int maxCryptoToken = 0;
        ...
        return maxCryptoToken == 1 || maxCryptoToken == 2;
    }

    public boolean isAes128Cbc() {
        if (this.mPrivacyCryptoList.isEmpty()) {
            return true;
        }
        int maxPrivacyToken = 0;
        for (int i = 0; i < this.mPrivacyCryptoList.size(); i++) {
            if (maxPrivacyToken < this.mPrivacyCryptoList.get(i).intValue()) {
                maxPrivacyToken = this.mPrivacyCryptoList.get(i).intValue();
            }
        }
        return maxPrivacyToken == 0;
    }

    public boolean isAes256Cbc() {
        if (this.mPrivacyCryptoList.isEmpty()) {
            return false;
        }
        int maxPrivacyToken = 0;
        ...
        return maxPrivacyToken == 1;
    }

    public boolean isPrivacyAes256GCM() {
        if (this.mPrivacyCryptoList.isEmpty()) {
            return false;
        }
        int maxPrivacyToken = 0;
        ...
        return maxPrivacyToken == 2;
    }

    public List<Integer> getCryptoInfoData() {
        return this.mCryptoList;
    }

    public List<Integer> getPrivacyCryptoInfoData() {
        return this.mPrivacyCryptoList;
    }

    public void clearCryptoInfoData() {
        this.mCryptoList.clear();
    }

    public void clearPrivacyCryptoInfoData() {
        this.mPrivacyCryptoList.clear();
    }

    public void addCryptoInfoData(int cryptoInfoData) {
        this.mCryptoList.add(Integer.valueOf(cryptoInfoData));
    }

    public void addPrivacyCryptoInfoData(int privacyCryptoInfoData) {
        this.mPrivacyCryptoList.add(Integer.valueOf(privacyCryptoInfoData));
    }
}
